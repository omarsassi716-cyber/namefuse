...
export function generateUsernames(
  ...
  while (usernamesSet.size < count && iterations < 3000) {
    ...
    if (filters) {
      finalCandidate = applyFilterConstraintsForcefully(finalCandidate, filters);
    }

    const formatted = formatUsername(finalCandidate, platform);

    // Apply advanced quality filtering and custom advanced filters before adding
    if (isValidUsernameQuality(formatted) && matchesAdvancedFilters(formatted, filters)) {
  ...
  if (usernamesSet.size < count) {
    ...
    while (usernamesSet.size < count && fallbackIdx < 500) {
      ...
      let name = `${designKey}_${styleKey.toLowerCase()}${fallbackIdx}`;
      if (filters) {
        name = applyFilterConstraintsForcefully(name, filters);
      }
      const formatted = formatUsername(name, platform);
      if (isValidUsernameQuality(formatted) && matchesAdvancedFilters(formatted, filters)) {
        usernamesSet.add(formatted);
      }
    }
...
export function matchesAdvancedFilters(name: string, filters?: AdvancedFilterOptions): boolean {
  if (!filters) return true;
  const lower = name.toLowerCase();
  
  if (filters.minLength !== undefined && name.length < filters.minLength) return false;
  if (filters.maxLength !== undefined && name.length > filters.maxLength) return false;
  
  if (filters.startsWith) {
    const sw = filters.startsWith.toLowerCase();
    if (!lower.startsWith(sw)) return false;
  }
  
  if (filters.endsWith) {
    const ew = filters.endsWith.toLowerCase();
    if (!lower.endsWith(ew)) return false;
  }
  
  if (filters.allowNumbers === false && /[0-9]/.test(name)) return false;
  if (filters.allowSymbols === false && /[._-]/.test(name)) return false;
  
  return true;
}

export function applyFilterConstraintsForcefully(name: string, filters?: AdvancedFilterOptions): string {
  if (!filters) return name;
  let result = name;
  
  if (filters.allowNumbers === false) {
    result = result.replace(/[0-9]/g, "");
  }
  if (filters.allowSymbols === false) {
    result = result.replace(/[._-]/g, "");
  }
  
  if (filters.startsWith) {
    const sw = filters.startsWith;
    if (!result.toLowerCase().startsWith(sw.toLowerCase())) {
      result = sw + result;
    }
  }
  
  if (filters.endsWith) {
    const ew = filters.endsWith;
    if (!result.toLowerCase().endsWith(ew.toLowerCase())) {
      result = result + ew;
    }
  }
...
export function calculateUsernameScore(
  ...
  uniqueness: number;
  socialFriendliness: number;
  domainFriendliness: number;
  overall: number;
} {
  const lower = name.toLowerCase();
  
  // 1. Originality & Uniqueness Scoring
  ...
  if (/[0-9]/.test(name)) uniqueness -= 10;
  // Multiple numbers hurts more
  const digitCount = (name.match(/\d/g) || []).length;
  if (digitCount > 2) uniqueness -= 8;
  
  // Having multiple symbols lowers pristine uniqueness
  const symbolCount = (name.match(/[._-]/g) || []).length;
  if (symbolCount > 1) uniqueness -= 8;

  // Length penalties/rewards for uniqueness
  if (lower.length > 15) uniqueness -= 5; // long handles feel standard
  if (lower.length < 5) uniqueness += 8;   // ultra short is highly original / rare
  ...
  // Penalize for heavy consonant clusters (too hard to pronounce)
  const consonants = "bcdfghjklmnpqrstvwxz";
  let consecutiveConsonantsCount = 0;
  for (let i = 0; i < lower.length; i++) {
    if (consonants.includes(lower[i])) {
      consecutiveConsonantsCount++;
      if (consecutiveConsonantsCount >= 3) {
        pronunciation -= 10;
      }
      if (consecutiveConsonantsCount >= 4) {
        pronunciation -= 15;
  ...
  if (/[0-9]/.test(name)) pronunciation -= 12; // Numbers are hard to speak out loud
  if (/[._-]/.test(name)) pronunciation -= 8;  // Special characters disrupt voice flow
  if (lower.length > 15) pronunciation -= 10;  // Long names are hard to say quickly

  // 5. Social Media Friendliness
  let socialFriendliness = 95;
  if (lower.length > 15) socialFriendliness -= 15; // too long for some bios
  if (/[._-]/.test(name)) socialFriendliness -= 8;  // some platforms don't allow symbols
  if (/[0-9]/.test(name)) socialFriendliness -= 5;  // feels less official or taken-placeholder
  // Check if character requirements for specific platforms are broken
  if (platform === "Roblox" && /[._-]/.test(name)) socialFriendliness -= 30;
  if (platform === "Twitch" && /[._-]/.test(name)) socialFriendliness -= 30;

  // 6. Domain Friendliness
  let domainFriendliness = 95;
  if (/[._]/.test(name)) domainFriendliness -= 40; // domains can't contain underscores, dots inside labels
  if (/-/.test(name)) domainFriendliness -= 15;   // hyphens are allowed but look cheap
  if (/[0-9]/.test(name)) domainFriendliness -= 20; // numbers make brand domains look spammy
  if (lower.length > 12) domainFriendliness -= 15;  // too long to buy/remember .com
  if (lower.length < 8 && !/[0-9._-]/.test(name)) domainFriendliness += 5; // short domain is gold

  // Clamp function to keep results elegant (between 65 and 99)
  const clamp = (val: number) => Math.max(65, Math.min(99, Math.round(val)));

  const uniqClamped = clamp(uniqueness);
  const brandClamped = clamp(brandability);
  const memoClamped = clamp(memorability);
  const pronClamped = clamp(pronunciation);
  const socClamped = clamp(socialFriendliness);
  const domClamped = clamp(domainFriendliness);
  
  const overall = Math.round((uniqClamped + brandClamped + memoClamped + pronClamped + socClamped + domClamped) / 6);

  return {
    originality: uniqClamped,
    brandability: brandClamped,
    memorability: memoClamped,
    pronunciation: pronClamped,
    uniqueness: uniqClamped,
    socialFriendliness: socClamped,
    domainFriendliness: domClamped,
    overall
  };
}
...
export function generateDisplayNames(
  ...
  while (namesSet.size < count && iterations < 3000) {
    ...
    let finalName = name;
    if (filters) {
      finalName = applyFilterConstraintsForcefully(finalName, filters);
    }
    ...
    // Quality check
    if (finalName && finalName.length >= 2 && finalName.length <= 32 && matchesAdvancedFilters(finalName, filters)) {
  ...
  if (namesSet.size < count) {
    ...
    while (namesSet.size < count && fallbackIdx < 500) {
      fallbackIdx++;
      let name = `${baseWord} ${styleKey} ${fallbackIdx}`;
      if (filters) {
        name = applyFilterConstraintsForcefully(name, filters);
      }
      if (platform === "Twitch") {
      ...
      if (name && name.length >= 2 && name.length <= 32 && matchesAdvancedFilters(name, filters)) {
        namesSet.add(name);
      }
    }
  }

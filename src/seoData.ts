...
function makeUniqueTitle(baseTitle: string, seen: Set<string>): string {
...
function makeUniqueDesc(baseDesc: string, seen: Set<string>): string {
...
for (const config of allConfigs) {
  ...
  // Guarantee 100% unique titles using elegant, CTR-optimized search modifiers
  const finalTitle = makeUniqueTitle(page.metaTitle, seenTitles);
  seenTitles.add(finalTitle);
  page.metaTitle = finalTitle;

  // Guarantee 100% unique descriptions using elegant CTAs
